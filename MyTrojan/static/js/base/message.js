...
Message.prototype.initElement = function () {
    ...
    this.wrapper.addClass("alert-dark");
    this.wrapper.addClass("shadow-sm");
    // 设置提示框整体样式
    this.wrapper.css({
...
Message.prototype.listenCloseEvent = function () {
    ...
    this.closeBtn.click(function () {
        _this.wrapper.stop().animate({"top": -_this.wrapperHeight});
    });
};
...
Message.prototype.show = function (message, type) {
    ...
    // 设置两秒后自动关闭
    this.wrapper.animate({"top": window.innerHeight / 10}, function () {
        setTimeout(function () {
            _this2.wrapper.animate({"top": -_this2.wrapperHeight}, {"queue": true});
        }, 2500);
    });
};
// 将对象绑定到 window 上
